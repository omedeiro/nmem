...
def nMemTestDie():
    ...
    F<<pr.route_smooth(nMem.ports[3], B2.ports[2], layer=2, width=(nMem.ports[3].width, 2))
    F<<pr.route_smooth(nMem.ports[4], B1.ports[2], layer=2, width=(nMem.ports[4].width, 2))
    B1.movex(-20)
    B2.movex(20)
    F<<D
...
def nMemArray_n2():
    ...
    for i in range(1,int(len(nmem.ports)/2)+1):
        F<<pr.route_smooth(nmem.ports[i], D.ports[i], path_type='Z', length1=20, length2=50, layer=1)
    ...
    for i in range(int(len(nmem.ports)/2)+1,len(nmem.ports)+1 ):
        ...
        F<<pr.route_sharp(nmem.ports[i], B1.ports[2], path_type='Z', length1=20, length2=50, layer=2)
...
def nMemArray_n4(n=4):
    D = om.die_cell_v3(ports = {'N':n, 'E':n, 'W':n, 'S':n},ports_ground=['E','S'], size2=(500,500))
    F = Device('nmem_array_cell')
    ...
    for i in range(1,2*n+1):
        F<<pr.route_smooth(nmem.ports[i], D.ports[i], path_type='Z', length1=50, length2=50, layer=1)
...
def nMemArray_n8(n=8):
    D = om.die_cell_v3(ports = {'N':n, 'E':n, 'W':n, 'S':n},ports_ground=['E','S'], size=(250*2*n,250*2*n), size2=(600,600))
    F = Device('nmem_array_cell')
    nmem = F<<om.memory_array(n,n)
    nmem.move(nmem.center, D.center)
...
def snspdArray(n=8):
    D = om.die_cell_v3(ports={'N':n, 'E':n, 'W':n, 'S':n},ports_ground=['E','S'], size=(250*2*n, 250*2*n), size2=(650,650), ground_width=300)
