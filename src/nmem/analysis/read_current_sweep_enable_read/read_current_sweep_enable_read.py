...
def plot_enable_write_sweep(ax: plt.Axes, dict_list: list[dict], colors=None):
    if colors is None:
...
def plot_enable_read_sweep(ax: plt.Axes, dict_list):
...
if __name__ == "__main__":
    ...
    ax: plt.Axes = axs[1, 0]
    plot_enable_read_sweep(ax, dict_list[::-1])

    ax: plt.Axes = axs[1, 1]
    plot_enable_read_temp(ax, enable_read_currents, read_temperatures)

    ax = axs[0, 0]
    plot_enable_write_sweep(ax, data_list2)
