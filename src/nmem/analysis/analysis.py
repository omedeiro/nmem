...
def create_combined_plot(data_dict: dict, save=False):
    ...
    plot_voltage_trace_averaged(ax[0], data_dict[4], "trace_write_avg", "#345F90")
    plot_voltage_trace_averaged(ax[1], data_dict[4], "trace_ewrite_avg", "#345F90")
    plot_voltage_trace_averaged(ax[2], data_dict[4], "trace_read0_avg", "#345F90")
    plot_voltage_trace_averaged(ax[2], data_dict[4], "trace_read1_avg", "#B3252C")
    plot_voltage_trace_averaged(ax[3], data_dict[4], "trace_eread_avg", "#345F90")
    ax[2].legend(["Read 0", "Read 1"], frameon=False)
    axhist = subfigs[0, 2].add_subplot()
    subfigs[0, 2].subplots_adjust(hspace=0.0)
    plot_voltage_hist(axhist, data_dict[3])

    axbits = subfigs[1, 1].subplots(4, 1)
    plot_voltage_trace_bitstream(axbits[0], data_dict[5], trace_name="trace_chan_out")
    plot_voltage_trace_bitstream(axbits[1], data_dict[6], trace_name="trace_chan_out")
    plot_voltage_trace_bitstream(axbits[2], data_dict[7], trace_name="trace_chan_out")
    plot_voltage_trace_bitstream(axbits[3], data_dict[8], trace_name="trace_chan_out")
    subfigs[1, 1].supylabel("Voltage [V]", x=0.05)
    subfigs[1, 1].supxlabel("Time [$\mu$s]")

    axdelay = subfigs[1, 2].add_subplot()
    plot_read_delay2(axdelay, data_dict)
...
def create_combined_plot_v4(data_dict: dict, save=False):
    ...
    fig.subplots_adjust(wspace=0.45)
    plot_voltage_hist(ax_dict["C"], data_dict[3])
