...
def plot_text_labels(xloc, yloc, ztotal):
    for x, y in zip(xloc, yloc):
        plt.text(x, y, f"{ztotal[y, x]:.2f}", fontsize=12, color="black", backgroundcolor="none", ha="center", va="center", weight="bold")
...
if __name__ == "__main__":
    ...
    for c in CELLS:
        ...
        if intercept != 0:
            ...
            write_heater_current = htron_heater_current(
                write_current, slope, intercept
            )
            read_heater_current = htron_heater_current(
                read_current, slope, intercept
            )
    ...
    plot_array(xloc, yloc, slope_array, "Slope [$\mu$A/$\mu$A]")
