...
def nTronTestDie2():
    ...
    D = pg.gridsweep(
        om.die_cell_v3,
        param_y={
            "text1": ["NTRON " + sub for sub in list(string.ascii_uppercase[0:N])]
        },
        param_x={"text2": list(string.digits[1 : N + 1])},
        param_defaults={
            "size": (500 * n, 500 * n),
    ...
    for i, s in zip(D.references, choke_offset):
        ...
        F << pr.route_smooth(
            nTron.ports[1], i.ports[1], layer=1, path_type="Z", length1=10, radius=15
        )
        F << pr.route_smooth(
            nTron.ports[1], i.ports[2], layer=1, path_type="Z", length1=10, radius=15
        )

        F << pr.route_smooth(
            nTron.ports[2], i.ports[3], layer=1, path_type="Z", length1=20, radius=15
        )

        F << pr.route_smooth(
            nTron.ports[3], i.ports[5], layer=1, path_type="Z", length1=10, radius=15
        )
        F << pr.route_smooth(
            nTron.ports[3], i.ports[6], layer=1, path_type="Z", length1=10, radius=15
        )

        F << pr.route_smooth(
            nTron.ports[4], i.ports[7], layer=1, path_type="Z", length1=60, radius=15
        )
        F << pr.route_smooth(
            nTron.ports[4], i.ports[8], layer=1, path_type="Z", length1=60, radius=15
        )
...
def htron4p(channel_width=0.1, heater_width=0.1, text1="A", text2="B"):
    ...
    D = pg.gridsweep(
        om.die_cell_v3,
        param_y={"text1": list(string.ascii_uppercase[0:3])},
    ...
    for i, s, r in zip(D.references, channel_widths, heater_widths):
        ...
        F << pr.route_smooth(
            ta1.ports[2], i.ports[1], path_type="Z", length1=1, radius=1.1, layer=1
        )
        F << pr.route_smooth(
            ta1.ports[2], i.ports[2], path_type="Z", length1=1, radius=1.1, layer=1
        )

        F << pr.route_smooth(
            ta2.ports[2], i.ports[3], path_type="Z", length1=1, radius=1.1, layer=1
        )
        F << pr.route_smooth(
            ta2.ports[2], i.ports[4], path_type="Z", length1=1, radius=1.1, layer=1
        )
        ...
        F << pr.route_smooth(
            t1.ports[1], i.ports[12], path_type="U", length1=2, radius=0.5, layer=2
        )
        F << pr.route_smooth(
            t1.ports[2], i.ports[11], path_type="U", length1=2, radius=0.5, layer=2
        )

        F << pr.route_smooth(
            t2.ports[1], i.ports[10], path_type="U", length1=2, radius=0.5, layer=2
        )
        F << pr.route_smooth(
            t2.ports[2], i.ports[9], path_type="U", length1=2, radius=0.5, layer=2
        )
...
def nTronTestImag():
    ...
    D << pg.gridsweep(
        qg.ntron,
        spacing=(10, 10),
        param_x={"choke_w": np.linspace(0.01, 0.1, 10)},
        param_y={"channel_w": np.tile(0.2, 20)},
    )
...
def nMemArray4():
    ...
    marks = D << qg.alignment_marks(
        locations=((-4500, -4500), (-4500, 4500), (4500, -4500), (4500, 4500)), layer=0
    )
    ...
    dut = D << pg.gridsweep(
        nMemArray,
        spacing=(0, 0),
        param_x={"text1": list(string.ascii_uppercase[0:N])},
        param_y={"text2": list(string.digits[1 : N + 1])},
        param_defaults={"n": 4},
    )
...
die_array = pg.gridsweep(
    om.basic_die,
    param_x={"text1": list(string.ascii_uppercase[0:7])},
    param_y={"text2": list(string.digits[1:8])},
    param_defaults={"size": (10e3, 10e3), "text_size": 400, "text_location": "S"},
    spacing=(0, 0),
)
