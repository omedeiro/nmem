...
if __name__ == "__main__":
    ...
    for key, ltsp in list(data_dict.items())[::3]:  # Limit to the first 10 items
        try:
            amp_uA = int(key.split("_")[-1].replace("u.raw", ""))
        except ValueError:
            continue
        if amp_uA < -350 or amp_uA > 360:
            continue  # only plot 20uA–80uA traces

        time = ltsp.get_time()
        left_current = ltsp.get_data("Ix(HL:drain)")
        ax.plot(time, left_current, label=f"Left branch current", color="C0")
        right_current = ltsp.get_data("Ix(HR:drain)")
    ...
    ichl = ltsp.get_data("I(ichl)")
    ichr = ltsp.get_data("I(ichr)")
    time = ltsp.get_time()
