...
def objective_write_enable(x, meas_dict: dict):
    meas_dict["write_current"] = x[0] * 1e-6
    meas_dict["enable_write_current"] = x[1] * 1e-6
    meas_dict["write_width"] = x[2]
    meas_dict["enable_write_width"] = x[3]
...
def run_optimize(meas_dict: dict):
    ...
    opt_result = gp_minimize(
        partial(objective_write_enable, meas_dict=meas_dict),
        space,
        n_calls=NUM_CALLS,
        verbose=True,
        x0=[201, 190, 88, 38, -5],
