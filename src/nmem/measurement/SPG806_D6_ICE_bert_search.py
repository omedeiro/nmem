...
b = nt.nTron(config)
##############################
sample_name = [b.sample_name, b.device_type,
               b.device_name, b.properties['Save File']['cell']]
...
if run1==True:
    ...
    file_path, time_str = qf.save(b.properties,
                                  measurement_name,
                                  save_dict)
    save_dict['time_str'] = time_str
    nm.plot_ber_sweep(save_dict, measurement_settings, file_path, 'enable_write_current', 'write_current', 'ber')
...
if run1==False:
    ...
    file_path, time_str = qf.save(b.properties,
                                  ...
                                  save_dict)
    save_dict['time_str'] = time_str
    nm.plot_ber_sweep(save_dict, measurement_settings, file_path, 'enable_read_current', 'read_current', 'ber')



t2 = time.time()
print(f'run time {(t2-t1)/60:.2f} minutes')
b.inst.scope.save_screenshot(f"{file_path}_scope_screenshot.png", white_background=False)
